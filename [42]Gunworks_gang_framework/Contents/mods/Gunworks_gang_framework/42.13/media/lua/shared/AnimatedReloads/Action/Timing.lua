local AnimatedReloadsUtils = require("AnimatedReloads/Utils")
...
---@nodiscard
---@return number
function AnimatedReloadsAction.getReloadSpeed(action)
...
---@nodiscard
---@return number
function AnimatedReloadsAction.getScaledReloadDelay(action, seconds)
    if seconds <= 0 then
        return seconds
    end

    return seconds / AnimatedReloadsAction.getReloadSpeed(action)
end
...
---@nodiscard
---@return number
function AnimatedReloadsAction.getActionDurationMs(action, seconds, fallbackMs)
    if seconds == nil then
        return fallbackMs
    end

    local scaledSeconds = AnimatedReloadsAction.getScaledReloadDelay(action, seconds)
...
---@param seconds number|nil
---@param callback fun()
function AnimatedReloadsAction.runAfter(action, seconds, callback)
    ...
    AnimatedReloadsUtils.runAfter(AnimatedReloadsAction.getScaledReloadDelay(action, seconds), callback)
end

---@param action ISInsertMagazine
---@nodiscard
---@return boolean
function AnimatedReloadsAction.shouldQueueShortRackAfterInsert(action)
...
---@nodiscard
---@return boolean
function AnimatedReloadsAction.shouldUseShortLoad(action, handler)
    if not action then
        return false
    end
    ...
    return AnimatedReloadsAction.shouldQueueShortRackAfterInsert(action)
end

---@param action ISInsertMagazine
---@param enabled boolean|nil
function AnimatedReloadsAction.setShortRackAfterInsert(action, enabled)
    if not action then
        return
    end
...
---@param handler AnimatedReloadsHandler|nil
---@return boolean
function AnimatedReloadsAction.handleShortRackStart(action, handler)
    ...
    action:setAnimVariable("WeaponReloadType", action.gun:getWeaponReloadType())
...
---@param action ISRackFirearm
function AnimatedReloadsAction.handleShortRackStop(action)
...
---@param action ISRackFirearm
function AnimatedReloadsAction.handleShortRackPerform(action)
    if action and action.useShortRack then
        action.character:clearVariable("isRackingShort")
    end
end
...
---@param action ISBaseTimedAction
function AnimatedReloadsAction.queueGunRackAfter(action)
...
---@param action ISBaseTimedAction
function AnimatedReloadsAction.setShortRackPending(action)
