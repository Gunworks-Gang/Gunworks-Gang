...
------------------------------------------------
-- M1 Garand "ping" sound and spent casing on empty fire
-------------------------------------------------
...
ISReloadWeaponAction.onShoot = function(player, weapon)
    if weapon:getFullType() == "MWA.M1_GARAND" then
        if weapon:isRoundChambered() and weapon:getCurrentAmmoCount() == 0 and weapon:isContainsClip() then
            ...
            if SpentCasingPhysics then
                SpentCasingPhysics.doSpawnCasing(player, weapon, {
                    forwardOffset = 0.30,
                    sideOffset = 0.10,
                    heightOffset = 0.45,
                    shellForce = 0.01,
